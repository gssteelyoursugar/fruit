<template>
	<view class="container">
		...
		<view class="container-img" v-if="orderObj.length == 0" :class="{ active: isActive, 'text-danger': hasError }">
			...
			<view class="color-border" @tap="goIndex">
				<text>去逛逛</text>
			</view>
		</view>
		...
		<checkbox-group @change="buyChange" >
			<view class="tui-cart-cell  tui-mtop" v-for="(item,index) in orderObj" :key="index">
				<view class="tui-activity">
					<view class="tui-bold">
						<image :src="item.url" mode="aspectFit" class="tui-head-log"></image> {{item.name}}
					</view>
					...
					<template v-slot:content>
						<view class="tui-goods-item">
							...
							<view class="tui-goods-info">
								<view class="tui-goods-title"> <text class="tag-tit">{{itemTwo.labelName}}</text>{{itemTwo.name}}</view>
