					...
					<view class="tui-order-btn">
						...
						<view class="tui-btn-ml" v-if="item.payStatus == 0">
							<view class="tui-btn-ml-pay" @tap="payGo(item.orderNumber)">
								<text>去支付</text><tui-countdown :time="item.time - item.createDate" :hours="false" borderColor="#fff" @end="endOfTime(item.id)"></tui-countdown>
							</view>
						</view>
		...
		<view  :class="[currentTab === 1 ? 'actineclass' : 'errorclass']">
			...
			<view class="tui-order-item" v-for="(item,index) in myOrderData" :key="index">
				...
				<view class="tui-order-btn">
					...
					<view class="tui-btn-ml" v-if="item.tradeStatus == 0">
						<view class="tui-btn-ml-pay" @tap="payGo(item.orderNumber)">
							<text>去支付</text><tui-countdown :time="(item.time - item.createDate) /15" :hours="false" borderColor="#fff" @end="endOfTime(item.id)"></tui-countdown>
						</view>
...
<script>
	...
	import {getMyOrder,imgurl,postAgainOrder,postOrderPay,postCancelOrder,postDelOrders,postConfirmOrder,postCancelPay} from '../../api/request.js'
